...
import { treatments } from "../treatments"; // adjust path if needed
...
export default async function TreatmentDetailPage({
  ...
  const isArrayPrices = Array.isArray(treatment.prices);
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
            ...
            <div className="flex flex-col gap-4">
              ...
              {treatment.gallery.slice(1, 3).map((img, idx) => (
                <div key={idx} className="h-60 rounded-xl overflow-hidden">
                  <img
                    src={img}
                    alt={`${treatment.title} ${idx + 2}`}
                    className="w-full h-full object-cover"
                  />
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div>
                ...
                {isArrayPrices ? (
                  <div className="bg-white rounded-xl border border-neutral-200 divide-y divide-gray-100">
                    {(
                      treatment.prices as { label: string; amount: string }[]
                    ).map((item, idx) => (
                  ...
                  <div className="bg-[#00adef33] rounded-xl p-4 text-center">
                    <p className="text-2xl font-bold text-[#005e83]">
                      {(treatment.prices as { single: string }).single}
                    </p>
                    <p className="text-sm text-neutral-600 mt-1">
                      {(treatment.prices as { note?: string }).note ||
                        "Best Price"}
                    </p>
                  </div>
                )}
            ...
            {treatment.packageContents &&
              treatment.packageContents.length > 0 && (
                <div className="mt-8 md:mt-0">
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                    <ul className="space-y-2">
                      {treatment.packageContents.map((item, idx) => (
                        <li
                          key={idx}
          ...
          <div className="mt-12 p-6 bg-white rounded-2xl shadow-sm border border-gray-100">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <Link
                ...
                href={`https://wa.me/6281234567890?text=Hello%20Bali%20Gift%20Spa,%20I%20would%20like%20to%20book%20the%20${encodeURIComponent(treatment.title)}%20treatment.`}
                target="_blank"
