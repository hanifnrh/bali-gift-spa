...
export default function ServicesPage() {
  ...
  // Get the initial tab from the URL or default to 'ala-carte'
  ...
  // Update the URL when the tab changes (so the back/forward buttons work)
  const handleTabChange = (tab: 'ala-carte' | 'gift-packages') => {
    setActiveTab(tab);
    // Push the new query parameter
    router.push(`/services?tab=${tab}`, { scroll: false });
  };

  // If the URL changes (e.g., user clicks back/forward), update the active tab
  useEffect(() => {
